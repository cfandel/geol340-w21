...
Each numbered folder contains a Jupyter Notebook stepping through the setup, run, and post-processing of a simple MODFLOW groundwater model, as well as all the necessary input and output files. An instructor-facing version and a student-facing version are included for each. The models increase in complexity by adding more MODFLOW packages. 
The folder titled "ocotillo" contains the materials for the final project: a more complex model that students must calibrate and use to forecast the impacts of proposed new groundwater pumping in a small agricultural valley.
